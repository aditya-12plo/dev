...
if (!$CheckFile) {
    ...
    $SQL = "SELECT A.ID, A.NO_UBAH_STATUS,A.TGL_UBAH_STATUS,A.KD_GUDANG_ASAL,A.KD_GUDANG_TUJUAN,B.NO_CONT,
    		B.UKURAN
            FROM t_ubah_status A INNER JOIN t_no_kontainer B ON A.NO_UBAH_STATUS=B.NO_UBAH_STATUS
			INNER JOIN app_user D ON A.ID_USER = D.ID
			INNER JOIN t_organisasi C ON D.KD_ORGANISASI = C.ID
            WHERE C.USERNAME_TPSONLINE_BC IS NOT NULL
                  AND C.PASSWORD_TPSONLINE_BC IS NOT NULL
                  AND A.KD_STATUS = '1'
            LIMIT 0,20";
